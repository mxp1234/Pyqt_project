...
class dataProcess(QMainWindow,Ui_MainWindow):
    ...
    def __init__(self) :
        ...
        self.button.clicked.connect(self.run)   #回调函数可以不带括号
        ...
        self.gridLayout.addWidget(self.cav1)
        ...
        self.verticalLayout_77 = QtWidgets.QVBoxLayout(self.widget_9)
        self.verticalLayout_77.addWidget(self.cav3)
    ...
    def whereFile(self):
        self.lineEdit.clear()
        initialDir = 'C:\\Users\\29961\\Desktop\\桌面文件'
        '''记得最后此处路径删除'''
    ...
    def run(self):
        ...
        '''
        此处可调用师兄处理数据的函数，可导入函数的数据包括上述各种私有属性，其类型一般都为ndarray
        '''
            ...
            if fileName:
                try:
                    ...
                    for i, line in enumerate(text_lines):
                        c.drawString(100, height - 100 - i * 15, line)
                    c.save()
                except Exception as e:
    ...
    def draw(self,data):
        ...
        ax1.plot(t, 180 / np.pi * data, 'r', label='data', linewidth=0.5)
...
# axe是fig 的一个子类，表示在fig对象的子图，axe有很多属性。但是注意最终draw命令        
    ...
    def draw_table3(self,data):
        self.fig3.clear()
        ax1 = self.fig3.add_subplot(111)
        # ax1.clear()  #把图中的数据线条清除，保留坐标轴等其他设置
        ax1.set_xlabel('t (sec)')
        ax1.set_ylabel('俯仰角')
        data1 = data[0]
        data2 = data[1]
        data3 = data[2]
        self.modified_states = data3
        
        # print("验证list=",list[0][0],list[0][1],list[1][0],list[1][1],list[2][0],list[2][1])
        
        ax1.plot(t,  data1[:,14], 'r', label='风洞表轨迹', linewidth=2,alpha=0.7)
        ax1.plot(t,  data2[:,14], 'b', label='真实轨迹', linewidth=2,alpha=0.7)
        ax1.plot(t,  data3[:,14], 'y', label='修正后轨迹', linewidth=2, alpha=0.7)
        ...
        def on_scroll(event):
            ...
            y_center = event.ydata  # 获取鼠标当前的 y 坐标
    ...
    def show_loss(self,list):
        ...
        if self.train.finished_flag == False:
            self.plainTextEdit.setPlainText(f"训练第{self.train.epoch+1}/{self.train.epoch_max}轮，总损失为 {self.train.total_loss} \n  CD通道预设气动相对偏差为 {self.cd_bias} ，绝对气动偏差为 {self.cd_bias2} \n CL通道预设气动相对偏差为 {self.cl_bias} ，绝对气动偏差为 {self.cl_bias2} \n  CY通道预设气动相对偏差为 {self.cy_bias} ，绝对气动偏差为 {self.cy_bias2} \n 估算CD通道气动相对偏差均值为 {cd_relative_deviation:.2f} , 绝对偏差均值为 {cd_absolute_deviation:.2f}  \n 估算CL通道气动相对偏差均值为 {cl_relative_deviation:.2f} , 绝对偏差均值为 {cl_absolute_deviation:.2f} \n 估算CY通道气动相对偏差均值为 {cy_relative_deviation:.2f} , 绝对偏差均值为 {cy_absolute_deviation:.2f}")
        else:
    ...
    def ui_customize(self):
        ...
        self.widget_9.setStyleSheet("QWidget { border: 10px solid rgb(190, 190, 190); }")
...
'''
为什么不设置回调函数，就没法执行self.str = self.lineEdit.text()这一句代码？
'''
...
class plotTable3(QThread):
    ...
    def caculate_img(self,list):
        ...
        self.data2 = self.real_states
        self.data1 = self.fly_states
        self.plot_table3.emit([self.data1,self.data2,self.data3])
